...
class main:
 def main():
  hyu = str(input(f"{kun}[{kan}•{kun}] {im}Select : "))
  if (hyu == '1') or (hyu == '01'):
   os.system("clear")
   bnr()
   main.start()
   ...
   print (f"""
{kan}Tools Brutal Spam adalah {kon}alat untuk menjahili teman dengan menggunakan pesan kode verifikasi yang akakn di kirim melalui sms, wa & telepon target
{kan}Happy Hacking, Let's Go...""")
   os.system("python brutal.py")
  elif (hyu == '4') or (hyu == '04'):
   os.system("git pull")
   main.main()
  else:
   print (f"{kun}[{kan}•{kun}] {kun}Serius dikit lahh ngen**d")
   main.main()
 def start():
  try:
      nom=str(input(f"{kun}[{kan}•{kun}] {im}Nomor (ex:85xx) : "))
...
except KeyboardInterrupt:print (f"{ken}[{kun}•{ken}] {kun}Program dihentikkan secara paksa")
